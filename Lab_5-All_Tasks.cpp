...
void alarmActivationUpdate()
{
    ...
    gas_ppm = gas_sensor_reading();
    if( gas_ppm > SAFE_GAS_LEVEL) {
        gasDetectorState = ON;
        alarmState = ON;
        if (latch_gas == 1){
            print_time();
...
void uartTask()
{
    ...
    if( uartUsb.readable() ) {
        ...
        case 'S':
            ...
            case 'G':
                gas_ppm = gas_sensor_reading();
                sprintf ( str, "Gas level:%.2f\r\n", gas_ppm);
                uartUsb.write( str , strlen(str) );
                if (gas_ppm < SAFE_GAS_LEVEL){
                    sprintf ( str, "Safe levels of gas\r\n");
                    uartUsb.write( str , strlen(str) );
                }else{
                    sprintf ( str, "Unsafe leves of gas\r\n");
                    uartUsb.write( str , strlen(str) ); 
                }
                break;
        default:
